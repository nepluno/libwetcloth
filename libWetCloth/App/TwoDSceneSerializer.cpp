...
void serialize_subprog( SerializePacket* packet )
{
    ...
    {
        ofs_fluid << "v " << std::setprecision(8) << packet->m_fluid_vertices[i](0) << " " << packet->m_fluid_vertices[i](1) << " " << packet->m_fluid_vertices[i](2) << " " << packet->m_fluid_radii[i] << std::endl;
    }
    ...
    {
        ofs_cloth << "v " << std::setprecision(8) << packet->m_dbl_face_cloth_vertices[i](0) << " " << packet->m_dbl_face_cloth_vertices[i](1) << " " << packet->m_dbl_face_cloth_vertices[i](2) << " "
        << packet->m_dbl_face_cloth_sat[i] << " " << packet->m_dbl_face_cloth_dir[i] << " " << packet->m_dbl_face_cloth_group[i] << " " << packet->m_dbl_face_cloth_vertices_rest[i](0) << " " << packet->m_dbl_face_cloth_vertices_rest[i](1) << " " << packet->m_dbl_face_cloth_vertices_rest[i](2) << " " << packet->m_dbl_face_cloth_vertices_central[i](0) << " " << packet->m_dbl_face_cloth_vertices_central[i](1) << " " << packet->m_dbl_face_cloth_vertices_central[i](2) << std::endl;
    }
    
    for(auto& f : packet->m_dbl_face_cloth_indices)
    {
        ofs_cloth << "f " << (f(0)+1) << " " << (f(1)+1) << " " << (f(2)+1) << std::endl;
    }
    ...
    {
        ofs_internal << "v " << std::setprecision(8) << v(0) << " " << v(1) << " " << v(2) << std::endl;
    }
    for(auto& f : packet->m_internal_indices)
    {
        ofs_internal << "f " << (f(0)+1) << " " << (f(1)+1) << " " << (f(2)+1) << std::endl;
    }
    ...
    {
        ofs_external << "v " << std::setprecision(8) << v(0) << " " << v(1) << " " << v(2) << std::endl;
    }
    for(auto& f : packet->m_external_indices)
    {
        ofs_external << "f " << (f(0)+1) << " " << (f(1)+1) << " " << (f(2)+1) << std::endl;
    }
    
    std::ofstream ofs_spring(packet->fn_springs.c_str());
    for(auto& v : packet->m_attach_spring_vertices)
    {
        ofs_spring << "v " << std::setprecision(8) << v(0) << " " << v(1) << " " << v(2) << std::endl;
    }
...
void TwoDSceneSerializer::serializeScene( TwoDScene& scene,
...
void TwoDSceneSerializer::serializePositionOnly( TwoDScene& scene, const std::string& fn_pos )
{
...
void TwoDSceneSerializer::loadPosOnly( TwoDScene& scene, std::ifstream& inputstream )
{
...
void TwoDSceneSerializer::updateHairs(const TwoDScene& scene, SerializePacket* data)
{
    ...
    std::partial_sum(hair_indicator.begin(), hair_indicator.end(), hair_indicator.begin());
...
void TwoDSceneSerializer::updateFluid(const TwoDScene& scene, SerializePacket* data)
{
...
void TwoDSceneSerializer::updateMesh(const TwoDScene& scene, SerializePacket* data)
{
    ...
    for( auto& ptr : fields )
    {
        ...
        ptr->render([&] (const std::vector<Vector3s>& vertices, const std::vector<Vector3i>& indices, const Eigen::Quaternion<scalar>& rot, const Vector3s& center, const scalar& sign) {
            if(sign > 0.0) {
                ...
                for(size_t i = 0; i < indices.size(); ++i) {
                    data->m_internal_indices.push_back( indices[i] + Vector3i(vert_base_idx, vert_base_idx, vert_base_idx) );
                }
            } else {
                ...
                for(size_t i = 0; i < indices.size(); ++i) {
                    Vector3i tri_inversed = Vector3i(indices[i](0), indices[i](2), indices[i](1));
                    data->m_external_indices.push_back( tri_inversed + Vector3i(vert_base_idx, vert_base_idx, vert_base_idx) );
                }
                ...
                for(size_t i = 0; i < indices.size(); ++i) {
                    data->m_external_indices.push_back( indices[i] + Vector3i(vert_base_idx, vert_base_idx, vert_base_idx) );
                }
            }
        });
    }
...
void TwoDSceneSerializer::updateAttachSprings(const TwoDScene& scene, SerializePacket* data)
{
    const VectorXs& x = scene.getX();
    const VectorXs& rest_x = scene.getRestPos();
...
void TwoDSceneSerializer::updateDoubleFaceCloth(const TwoDScene& scene, SerializePacket* data)
{
    const VectorXs& x = scene.getX();
    const VectorXs& rest_x = scene.getRestPos();
    const std::vector<int> group = scene.getParticleGroup();
    ...
    std::partial_sum(cloth_indicator.begin(), cloth_indicator.end(), cloth_indicator.begin());
    const int num_single_cloth_verts = cloth_indicator[cloth_indicator.size() - 1];
    ...
    const int total_verts = num_single_cloth_verts * 2 + num_loop_verts * (num_cloth_edge_discretization - 1);
    const int total_faces = faces.rows() * 2 + num_loop_verts * num_cloth_edge_discretization * 2;
    ...
    threadutils::for_each(0, num_soft_elasto, [&] (int pidx) {
        ...
        if(p2f.size() > 0) {
            ...
            data->m_dbl_face_cloth_vertices_central[ num_single_cloth_verts + new_pidx ] = x.segment<3>(pidx * 4);
            ...
            data->m_dbl_face_cloth_group[ num_single_cloth_verts + new_pidx ] = group[pidx];
            data->m_dbl_face_cloth_sat[ new_pidx ] = fluid_vol(pidx) / std::max(1e-16, vol(pidx));
            data->m_dbl_face_cloth_sat[ num_single_cloth_verts + new_pidx ] = fluid_vol(pidx) / std::max(1e-16, vol(pidx));
        }
    });
    ...
    threadutils::for_each(0, (int) faces.rows(), [&] (int fidx) {
        ...
        data->m_dbl_face_cloth_indices[fidx] = Vector3i(new_pidx_0, new_pidx_2, new_pidx_1);
        data->m_dbl_face_cloth_indices[fidx + faces.rows()] = Vector3i(new_pidx_0 + num_single_cloth_verts, new_pidx_1 + num_single_cloth_verts, new_pidx_2 + num_single_cloth_verts );
    });
    ...
    for(int iCloth = 0; iCloth < num_clothes; ++iCloth) {
        ...
        const int cloth_edge_vert_base = base_vtx + lv_base[iCloth] * (num_cloth_edge_discretization - 1);
        const int cloth_edge_face_base = base_faces + lv_base[iCloth] * num_cloth_edge_discretization * 2;
        ...
        threadutils::for_each(0, num_bverts, [&] (int idx) {
            ...
            const Vector3s e = (x.segment<3>(next_pidx * 4) - x.segment<3>(prev_pidx * 4)).normalized();
            ...
            for(int i = 0; i < num_cloth_edge_discretization; ++i) {
                int v_prev = (i == 0) ? (cloth_indicator[pidx] - 1) : (cloth_edge_vert_base + idx * (num_cloth_edge_discretization - 1) + (i - 1));
                int v_next = (i == num_cloth_edge_discretization - 1) ? (num_single_cloth_verts + cloth_indicator[pidx] - 1) : (cloth_edge_vert_base + idx * (num_cloth_edge_discretization - 1) + i);
                
                int vn_prev = (i == 0) ? (cloth_indicator[next_pidx] - 1) : (cloth_edge_vert_base + next_idx * (num_cloth_edge_discretization - 1) + (i - 1));
                int vn_next = (i == num_cloth_edge_discretization - 1) ? (num_single_cloth_verts + cloth_indicator[next_pidx] - 1) : (cloth_edge_vert_base + next_idx * (num_cloth_edge_discretization - 1) + i);
                
                data->m_dbl_face_cloth_indices[cloth_edge_face_base + idx * (num_cloth_edge_discretization * 2) + i * 2 + 0] = Vector3i(v_prev, vn_next, v_next);
                data->m_dbl_face_cloth_indices[cloth_edge_face_base + idx * (num_cloth_edge_discretization * 2) + i * 2 + 1] = Vector3i(v_prev, vn_prev, vn_next);
            }
        });
    }
}
