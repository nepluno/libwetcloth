    ...
    if (denominator <= 0. || isSmall(denominator)) {
      ...
      if (denominator <= 0.) {
        std::cerr << "CurvatureBinormals::compute() denominator == "
                  << denominator << " at vertex " << vtx << " t1 = " << t1
                  << " t2 = " << t2 << std::endl;

        m_value[vtx] = Vec3::Constant(
            std::numeric_limits<scalar>::infinity());  // Should not be
                                                       // accepted.
      } else {
...
void TrigThetas::vdSinCos(const int n, const double a[], double r1[],
                          double r2[]) {
