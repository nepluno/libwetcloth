...
RoundCylinder::RoundCylinder(int N, int M, const double& ra, const double& rb,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    indices[i + N] =
        Vector3i(1, 2 + (i + N) * (M + 1) + M, 2 + (j + N) * (M + 1) + M);
  }
  ...
  for (int i = 0; i < N; ++i) {
    int j = (i + 1) % N;
    indices[base_idx + i * 2 + 0] =
        Vector3i(2 + j * (M + 1), 2 + (i + N) * (M + 1), 2 + i * (M + 1));
    indices[base_idx + i * 2 + 1] =
        Vector3i(2 + j * (M + 1), 2 + (j + N) * (M + 1), 2 + (i + N) * (M + 1));
  }
  ...
  for (int i = 0; i < N; ++i) {
    int j = (i + 1) % N;
    for (int k = 0; k < M; ++k) {
      indices[base_idx + (i * M + k) * 2 + 0] =
          Vector3i(2 + j * (M + 1) + (k + 1), 2 + i * (M + 1) + k,
                   2 + i * (M + 1) + (k + 1));
      indices[base_idx + (i * M + k) * 2 + 1] = Vector3i(
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int k = 0; k < M; ++k) {
      indices[base_idx + (i * M + k) * 2 + 0] =
          Vector3i(2 + (j + N) * (M + 1) + k, 2 + (i + N) * (M + 1) + (k + 1),
                   2 + (i + N) * (M + 1) + k);
      indices[base_idx + (i * M + k) * 2 + 1] =
          Vector3i(2 + (j + N) * (M + 1) + k, 2 + (j + N) * (M + 1) + (k + 1),
                   2 + (i + N) * (M + 1) + (k + 1));
    }
  }
}
