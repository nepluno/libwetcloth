  ...
  if (phi0 < 0) {
    if (phi1 < 0) {
      if (phi2 < 0)
        ...
        return 1 - sorted_triangle_fraction(phi2, phi0, phi1);
    } else if (phi2 < 0)
      return 1 - sorted_triangle_fraction(phi1, phi2, phi0);
    else
      return sorted_triangle_fraction(phi0, phi1, phi2);
  } else if (phi1 < 0) {
    if (phi2 < 0)
      return 1 - sorted_triangle_fraction(phi0, phi1, phi2);
    else
      return sorted_triangle_fraction(phi1, phi2, phi0);
  } else if (phi2 < 0)
    return sorted_triangle_fraction(phi2, phi0, phi1);
  else
    return 0;
}
...
float area_fraction(float phi00, float phi10, float phi01, float phi11) {
  ...
  return (area_fraction(phi00, phi10, phimid) +
          ...
          area_fraction(phi01, phi00, phimid)) /
         4;
}
...
static T sorted_prism_fraction(T phi0, T phi1, T phi2, T phi3) {
  T a = phi0 / (phi0 - phi2), b = phi0 / (phi0 - phi3),
    c = phi1 / (phi1 - phi3), d = phi1 / (phi1 - phi2);
...
float volume_fraction(float phi0, float phi1, float phi2, float phi3) {
  ...
  if (phi3 <= 0)
    ...
    return 1 - sorted_tet_fraction(phi3, phi2, phi1, phi0);
  else if (phi1 <= 0)
    return sorted_prism_fraction(phi0, phi1, phi2, phi3);
  else if (phi0 <= 0)
    return sorted_tet_fraction(phi0, phi1, phi2, phi3);
  else
    return 0;
}
