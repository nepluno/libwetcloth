...
void computePorePressureGrads(const TwoDScene& scene,
  ...
  const std::vector<VectorXs>& node_pore_pressure =
      scene.getNodePorePressureP();
  ...
  const std::vector<VectorXi>& node_pressure_index_x =
      scene.getNodePressureIndexX();
  const std::vector<VectorXi>& node_pressure_index_y =
      scene.getNodePressureIndexY();
  const std::vector<VectorXi>& node_pressure_index_z =
      scene.getNodePressureIndexZ();
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_rhs_x; ++i) {
      if (node_vol_x[bucket_idx](i) > 0.0 &&
          node_weight_x[bucket_idx](i) > 0.0) {
        const Vector4i& indices =
            node_pressure_index_x[bucket_idx].segment<4>(i * 4);
        const scalar pp0 = (indices[0] < 0 || indices[1] < 0)
                               ? 0.0
                               : node_pore_pressure[indices[0]][indices[1]];
        const scalar pp1 = (indices[2] < 0 || indices[3] < 0)
                               ? 0.0
                               : node_pore_pressure[indices[2]][indices[3]];

        const scalar pressure_grad = (pp1 - pp0) / dx;
        scalar term = pressure_grad * node_vol_x[bucket_idx](i) * dt *
                      node_sat_x[bucket_idx](i) * node_weight_x[bucket_idx](i) *
                      mathutils::get_rotated_pore_coeff_x(
                          orient_x[bucket_idx].segment<3>(i * 3),
                          shape_factor_x[bucket_idx](i));
    ...
    for (int i = 0; i < num_rhs_y; ++i) {
      if (node_vol_y[bucket_idx](i) > 0.0 &&
          node_weight_y[bucket_idx](i) > 0.0) {
        const Vector4i& indices =
            node_pressure_index_y[bucket_idx].segment<4>(i * 4);
        const scalar pp0 = (indices[0] < 0 || indices[1] < 0)
                               ? 0.0
                               : node_pore_pressure[indices[0]][indices[1]];
        const scalar pp1 = (indices[2] < 0 || indices[3] < 0)
        ...
        const scalar pressure_grad = (pp1 - pp0) / dx;
        scalar term = pressure_grad * node_vol_y[bucket_idx](i) * dt *
                      node_sat_y[bucket_idx](i) * node_weight_y[bucket_idx](i) *
                      mathutils::get_rotated_pore_coeff_y(
                          orient_y[bucket_idx].segment<3>(i * 3),
                          shape_factor_y[bucket_idx](i));
    ...
    for (int i = 0; i < num_rhs_z; ++i) {
      if (node_vol_z[bucket_idx](i) > 0.0 &&
          node_weight_z[bucket_idx](i) > 0.0) {
        const Vector4i& indices =
            node_pressure_index_z[bucket_idx].segment<4>(i * 4);
        const scalar pp0 = (indices[0] < 0 || indices[1] < 0)
                               ? 0.0
                               : node_pore_pressure[indices[0]][indices[1]];
        const scalar pp1 = (indices[2] < 0 || indices[3] < 0)
        ...
        const scalar pressure_grad = (pp1 - pp0) / dx;
        scalar term = pressure_grad * node_vol_z[bucket_idx](i) * dt *
                      node_sat_z[bucket_idx](i) * node_weight_z[bucket_idx](i) *
                      mathutils::get_rotated_pore_coeff_z(
                          orient_z[bucket_idx].segment<3>(i * 3),
                          shape_factor_z[bucket_idx](i));
        ;
...
void constructNodeIncompressibleCondition(
  ...
  const std::vector<VectorXi>& pressure_neighbors =
      scene.getPressureNeighbors();
  ...
  const std::vector<VectorXs>& node_solid_weight_x =
      scene.getNodeSolidWeightX();
  const std::vector<VectorXs>& node_solid_weight_y =
      scene.getNodeSolidWeightY();
  const std::vector<VectorXs>& node_solid_weight_z =
      scene.getNodeSolidWeightZ();
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_rhs; ++i) {
      ...
      if (bucket_idx_left >= 0 && node_idx_left >= 0) {
        ...
        bucket_node_rhs(i) +=
            ((node_fluid_vel_x[bucket_idx_left][node_idx_left] *
                  (1.0 - node_psi_x[bucket_idx_left][node_idx_left]) +
              node_elasto_vel_x[bucket_idx_left][node_idx_left] *
                  node_psi_x[bucket_idx_left][node_idx_left]) *
                 w +
             node_solid_vel_x[bucket_idx_left][node_idx_left] * (1.0 - w)) /
            dx;
      }
      ...
      if (bucket_idx_right >= 0 && node_idx_right >= 0) {
        ...
        bucket_node_rhs(i) -=
            ((node_fluid_vel_x[bucket_idx_right][node_idx_right] *
                  (1.0 - node_psi_x[bucket_idx_right][node_idx_right]) +
              node_elasto_vel_x[bucket_idx_right][node_idx_right] *
                  node_psi_x[bucket_idx_right][node_idx_right]) *
                 w +
             node_solid_vel_x[bucket_idx_right][node_idx_right] * (1.0 - w)) /
            dx;
      }
      ...
      if (bucket_idx_bottom >= 0 && node_idx_bottom >= 0) {
        const scalar& w =
            node_solid_weight_y[bucket_idx_bottom][node_idx_bottom];
        bucket_node_rhs(i) +=
            ((node_fluid_vel_y[bucket_idx_bottom][node_idx_bottom] *
                  (1.0 - node_psi_y[bucket_idx_bottom][node_idx_bottom]) +
              node_elasto_vel_y[bucket_idx_bottom][node_idx_bottom] *
                  node_psi_y[bucket_idx_bottom][node_idx_bottom]) *
                 w +
             node_solid_vel_y[bucket_idx_bottom][node_idx_bottom] * (1.0 - w)) /
            dx;
      }
      ...
      if (bucket_idx_top >= 0 && node_idx_top >= 0) {
        ...
        bucket_node_rhs(i) -=
            ((node_fluid_vel_y[bucket_idx_top][node_idx_top] *
                  (1.0 - node_psi_y[bucket_idx_top][node_idx_top]) +
              node_elasto_vel_y[bucket_idx_top][node_idx_top] *
                  node_psi_y[bucket_idx_top][node_idx_top]) *
                 w +
             node_solid_vel_y[bucket_idx_top][node_idx_top] * (1.0 - w)) /
            dx;
      }
      ...
      if (bucket_idx_near >= 0 && node_idx_near >= 0) {
        ...
        bucket_node_rhs(i) +=
            ((node_fluid_vel_z[bucket_idx_near][node_idx_near] *
                  (1.0 - node_psi_z[bucket_idx_near][node_idx_near]) +
              node_elasto_vel_z[bucket_idx_near][node_idx_near] *
                  node_psi_z[bucket_idx_near][node_idx_near]) *
                 w +
             node_solid_vel_z[bucket_idx_near][node_idx_near] * (1.0 - w)) /
            dx;
      }
      ...
      if (bucket_idx_far >= 0 && node_idx_far >= 0) {
        ...
        bucket_node_rhs(i) -=
            ((node_fluid_vel_z[bucket_idx_far][node_idx_far] *
                  (1.0 - node_psi_z[bucket_idx_far][node_idx_far]) +
              node_elasto_vel_z[bucket_idx_far][node_idx_far] *
                  node_psi_z[bucket_idx_far][node_idx_far]) *
                 w +
             node_solid_vel_z[bucket_idx_far][node_idx_far] * (1.0 - w)) /
            dx;
      }
    }
  });
...
void solveNodePressure(
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_nodes_p; ++i) {
      ...
      if (node_liquid_phi[bucket_idx][i] < 0) {
        ...
        if ((bucket_idx_left >= 0 && node_idx_left >= 0 &&
             node_solid_weight_x[bucket_idx_left][node_idx_left] > 0) ||
            (bucket_idx_right >= 0 && node_idx_right >= 0 &&
             node_solid_weight_x[bucket_idx_right][node_idx_right] > 0) ||
            (bucket_idx_bottom >= 0 && node_idx_bottom >= 0 &&
             node_solid_weight_y[bucket_idx_bottom][node_idx_bottom] > 0) ||
            (bucket_idx_top >= 0 && node_idx_top >= 0 &&
             node_solid_weight_y[bucket_idx_top][node_idx_top] > 0) ||
            (bucket_idx_near >= 0 && node_idx_near >= 0 &&
             node_solid_weight_z[bucket_idx_near][node_idx_near] > 0) ||
            (bucket_idx_far >= 0 && node_idx_far >= 0 &&
             node_solid_weight_z[bucket_idx_far][node_idx_far] > 0)) {
  ...
  buckets.for_each_bucket(
      [&](int bucket_idx) { pressure[bucket_idx].setZero(); });
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_nodes_p; ++i) {
      if (node_global_indices[bucket_idx][i] >= 0) {
        if (bucket_idx > 0) {
          node_global_indices[bucket_idx][i] +=
        ...
        dof_ijk[global_idx] =
            Vector3i(handle(0) * bucket_num_cell + local_handle(0),
    ...
    if (bucket_idx_left >= 0 && node_idx_left >= 0) {
      ...
      rhs[dof_idx] +=
          ((node_fluid_vel_x[bucket_idx_left][node_idx_left] *
            ...
            node_elasto_vel_x[bucket_idx_left][node_idx_left] *
                node_psi_sf_x[bucket_idx_left][node_idx_left]) *
               w +
           node_solid_vel_x[bucket_idx_left][node_idx_left] * (1.0 - w)) /
          dx;
      ...
      if (w > 0.0) {
        ...
        term *= (1.0 - node_psi_x[bucket_idx_left][node_idx_left]) *
                    node_inv_C_x[bucket_idx_left][node_idx_left] *
                    (node_fluid_vol_x[bucket_idx_left][node_idx_left] +
                     node_elasto_vol_x[bucket_idx_left][node_idx_left] *
                         node_mshdvm_hdvm_x[bucket_idx_left][node_idx_left]) +
                node_psi_x[bucket_idx_left][node_idx_left] *
                    node_inv_Cs_x[bucket_idx_left][node_idx_left] *
                    (node_elasto_vol_x[bucket_idx_left][node_idx_left] +
                     node_fluid_vol_x[bucket_idx_left][node_idx_left] *
                         node_mfhdvm_hdvm_x[bucket_idx_left][node_idx_left]);

        const int bucket_pressure_left =
            node_pressure_index_x[bucket_idx_left][node_idx_left * 4 + 0];
        const int node_pressure_left =
            node_pressure_index_x[bucket_idx_left][node_idx_left * 4 + 1];

        if (bucket_pressure_left >= 0 && node_pressure_left >= 0) {
          const scalar left_phi =
              node_liquid_phi[bucket_pressure_left][node_pressure_left];

          if (left_phi < 0.0) {
            const int dof_left =
                node_global_indices[bucket_pressure_left][node_pressure_left];
            assert(dof_left >= 0);
            ...
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, left_phi),
                         theta_criterion);
    ...
    if (bucket_idx_right >= 0 && node_idx_right >= 0) {
      ...
      rhs[dof_idx] -=
          ((node_fluid_vel_x[bucket_idx_right][node_idx_right] *
            ...
            node_elasto_vel_x[bucket_idx_right][node_idx_right] *
                node_psi_sf_x[bucket_idx_right][node_idx_right]) *
               w +
           node_solid_vel_x[bucket_idx_right][node_idx_right] * (1.0 - w)) /
          dx;
      ...
      if (w > 0.0) {
        ...
        term *= (1.0 - node_psi_x[bucket_idx_right][node_idx_right]) *
                    node_inv_C_x[bucket_idx_right][node_idx_right] *
                    (node_fluid_vol_x[bucket_idx_right][node_idx_right] +
                     node_elasto_vol_x[bucket_idx_right][node_idx_right] *
                         node_mshdvm_hdvm_x[bucket_idx_right][node_idx_right]) +
                node_psi_x[bucket_idx_right][node_idx_right] *
                    node_inv_Cs_x[bucket_idx_right][node_idx_right] *
                    (node_elasto_vol_x[bucket_idx_right][node_idx_right] +
                     node_fluid_vol_x[bucket_idx_right][node_idx_right] *
                         node_mfhdvm_hdvm_x[bucket_idx_right][node_idx_right]);

        const int bucket_pressure_right =
            node_pressure_index_x[bucket_idx_right][node_idx_right * 4 + 2];
        const int node_pressure_right =
            node_pressure_index_x[bucket_idx_right][node_idx_right * 4 + 3];

        if (bucket_pressure_right >= 0 && node_pressure_right >= 0) {
          const scalar right_phi =
              node_liquid_phi[bucket_pressure_right][node_pressure_right];

          if (right_phi < 0.0) {
            const int dof_right =
                node_global_indices[bucket_pressure_right][node_pressure_right];
            assert(dof_right >= 0);
            ...
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, right_phi),
                         theta_criterion);
            matrix.add_to_element(dof_idx, dof_idx, term / theta);
          }
        }
    ...
    if (bucket_idx_bottom >= 0 && node_idx_bottom >= 0) {
      ...
      rhs[dof_idx] +=
          ((node_fluid_vel_y[bucket_idx_bottom][node_idx_bottom] *
            ...
            node_elasto_vel_y[bucket_idx_bottom][node_idx_bottom] *
                node_psi_sf_y[bucket_idx_bottom][node_idx_bottom]) *
               w +
           node_solid_vel_y[bucket_idx_bottom][node_idx_bottom] * (1.0 - w)) /
          dx;
      ...
      if (w > 0.0) {
        ...
        term *=
            (1.0 - node_psi_y[bucket_idx_bottom][node_idx_bottom]) *
                node_inv_C_y[bucket_idx_bottom][node_idx_bottom] *
                (node_fluid_vol_y[bucket_idx_bottom][node_idx_bottom] +
                 node_elasto_vol_y[bucket_idx_bottom][node_idx_bottom] *
                     node_mshdvm_hdvm_y[bucket_idx_bottom][node_idx_bottom]) +
            node_psi_y[bucket_idx_bottom][node_idx_bottom] *
                node_inv_Cs_y[bucket_idx_bottom][node_idx_bottom] *
                (node_elasto_vol_y[bucket_idx_bottom][node_idx_bottom] +
                 node_fluid_vol_y[bucket_idx_bottom][node_idx_bottom] *
                     node_mfhdvm_hdvm_y[bucket_idx_bottom][node_idx_bottom]);

        const int bucket_pressure_bottom =
            node_pressure_index_y[bucket_idx_bottom][node_idx_bottom * 4 + 0];
        const int node_pressure_bottom =
            node_pressure_index_y[bucket_idx_bottom][node_idx_bottom * 4 + 1];

        if (bucket_pressure_bottom >= 0 && node_pressure_bottom >= 0) {
          const scalar bottom_phi =
              node_liquid_phi[bucket_pressure_bottom][node_pressure_bottom];

          if (bottom_phi < 0.0) {
            const int dof_bottom = node_global_indices[bucket_pressure_bottom]
            ...
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, bottom_phi),
                         theta_criterion);
            matrix.add_to_element(dof_idx, dof_idx, term / theta);
          }
        }
    ...
    if (bucket_idx_top >= 0 && node_idx_top >= 0) {
      ...
      rhs[dof_idx] -=
          ((node_fluid_vel_y[bucket_idx_top][node_idx_top] *
            ...
            node_elasto_vel_y[bucket_idx_top][node_idx_top] *
                node_psi_sf_y[bucket_idx_top][node_idx_top]) *
               w +
           node_solid_vel_y[bucket_idx_top][node_idx_top] * (1.0 - w)) /
          dx;
      ...
      if (w > 0.0) {
        ...
        term *= (1.0 - node_psi_y[bucket_idx_top][node_idx_top]) *
                    node_inv_C_y[bucket_idx_top][node_idx_top] *
                    (node_fluid_vol_y[bucket_idx_top][node_idx_top] +
                     node_elasto_vol_y[bucket_idx_top][node_idx_top] *
                         node_mshdvm_hdvm_y[bucket_idx_top][node_idx_top]) +
                node_psi_y[bucket_idx_top][node_idx_top] *
                    node_inv_Cs_y[bucket_idx_top][node_idx_top] *
                    (node_elasto_vol_y[bucket_idx_top][node_idx_top] +
                     node_fluid_vol_y[bucket_idx_top][node_idx_top] *
                         node_mfhdvm_hdvm_y[bucket_idx_top][node_idx_top]);

        const int bucket_pressure_top =
            node_pressure_index_y[bucket_idx_top][node_idx_top * 4 + 2];
        const int node_pressure_top =
            node_pressure_index_y[bucket_idx_top][node_idx_top * 4 + 3];

        if (bucket_pressure_top >= 0 && node_pressure_top >= 0) {
          const scalar top_phi =
              node_liquid_phi[bucket_pressure_top][node_pressure_top];

          if (top_phi < 0.0) {
            const int dof_top =
                node_global_indices[bucket_pressure_top][node_pressure_top];
            assert(dof_top >= 0);
            ...
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, top_phi),
                         theta_criterion);
            matrix.add_to_element(dof_idx, dof_idx, term / theta);
          }
        }
    ...
    if (bucket_idx_near >= 0 && node_idx_near >= 0) {
      ...
      rhs[dof_idx] +=
          ((node_fluid_vel_z[bucket_idx_near][node_idx_near] *
            ...
            node_elasto_vel_z[bucket_idx_near][node_idx_near] *
                node_psi_sf_z[bucket_idx_near][node_idx_near]) *
               w +
           node_solid_vel_z[bucket_idx_near][node_idx_near] * (1.0 - w)) /
          dx;
      ...
      if (w > 0.0) {
        ...
        term *= (1.0 - node_psi_z[bucket_idx_near][node_idx_near]) *
                    node_inv_C_z[bucket_idx_near][node_idx_near] *
                    (node_fluid_vol_z[bucket_idx_near][node_idx_near] +
                     node_elasto_vol_z[bucket_idx_near][node_idx_near] *
                         node_mshdvm_hdvm_z[bucket_idx_near][node_idx_near]) +
                node_psi_z[bucket_idx_near][node_idx_near] *
                    node_inv_Cs_z[bucket_idx_near][node_idx_near] *
                    (node_elasto_vol_z[bucket_idx_near][node_idx_near] +
                     node_fluid_vol_z[bucket_idx_near][node_idx_near] *
                         node_mfhdvm_hdvm_z[bucket_idx_near][node_idx_near]);

        const int bucket_pressure_near =
            node_pressure_index_z[bucket_idx_near][node_idx_near * 4 + 0];
        const int node_pressure_near =
            node_pressure_index_z[bucket_idx_near][node_idx_near * 4 + 1];

        if (bucket_pressure_near >= 0 && node_pressure_near >= 0) {
          const scalar near_phi =
              node_liquid_phi[bucket_pressure_near][node_pressure_near];

          if (near_phi < 0.0) {
            const int dof_near =
                node_global_indices[bucket_pressure_near][node_pressure_near];
            assert(dof_near >= 0);
            ...
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, near_phi),
                         theta_criterion);
            matrix.add_to_element(dof_idx, dof_idx, term / theta);
          }
        }
    ...
    if (bucket_idx_far >= 0 && node_idx_far >= 0) {
      ...
      rhs[dof_idx] -=
          ((node_fluid_vel_z[bucket_idx_far][node_idx_far] *
            ...
            node_elasto_vel_z[bucket_idx_far][node_idx_far] *
                node_psi_sf_z[bucket_idx_far][node_idx_far]) *
               w +
           node_solid_vel_z[bucket_idx_far][node_idx_far] * (1.0 - w)) /
          dx;
      ...
      if (w > 0.0) {
        ...
        term *= (1.0 - node_psi_z[bucket_idx_far][node_idx_far]) *
                    node_inv_C_z[bucket_idx_far][node_idx_far] *
                    (node_fluid_vol_z[bucket_idx_far][node_idx_far] +
                     node_elasto_vol_z[bucket_idx_far][node_idx_far] *
                         node_mshdvm_hdvm_z[bucket_idx_far][node_idx_far]) +
                node_psi_z[bucket_idx_far][node_idx_far] *
                    node_inv_Cs_z[bucket_idx_far][node_idx_far] *
                    (node_elasto_vol_z[bucket_idx_far][node_idx_far] +
                     node_fluid_vol_z[bucket_idx_far][node_idx_far] *
                         node_mfhdvm_hdvm_z[bucket_idx_far][node_idx_far]);

        const int bucket_pressure_far =
            node_pressure_index_z[bucket_idx_far][node_idx_far * 4 + 2];
        const int node_pressure_far =
            node_pressure_index_z[bucket_idx_far][node_idx_far * 4 + 3];

        if (bucket_pressure_far >= 0 && node_pressure_far >= 0) {
          const scalar far_phi =
              node_liquid_phi[bucket_pressure_far][node_pressure_far];

          if (far_phi < 0.0) {
            const int dof_far =
                node_global_indices[bucket_pressure_far][node_pressure_far];
            assert(dof_far >= 0);
            ...
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, far_phi),
                         theta_criterion);
            matrix.add_to_element(dof_idx, dof_idx, term / theta);
          }
        }
  ...
  success = AMGPCGSolveSparse(matrix, rhs, result, dof_ijk, criterion, maxiters,
                              tolerance, iterations, ni, nj, nk);
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_pressure; ++i) {
      ...
      if (bucket_idx_left >= 0 && node_idx_left >= 0) {
        ...
        term *= node_fluid_vol_x[bucket_idx_left][node_idx_left] *
                    (1.0 - node_psi_x[bucket_idx_left][node_idx_left]) *
                    node_inv_mdv_x[bucket_idx_left][node_idx_left] +
                node_elasto_vol_x[bucket_idx_left][node_idx_left] *
                    node_psi_x[bucket_idx_left][node_idx_left] *
                    node_inv_mdvs_x[bucket_idx_left][node_idx_left];
        ...
        if (bucket_pressure_left >= 0 && node_pressure_left >= 0) {
          ...
          if (left_phi < 0.0) {
            bucket_out(i) +=
                term * (node_vec[bucket_idx][i] -
                        node_vec[bucket_pressure_left][node_pressure_left]);
          } else {
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, left_phi),
                         theta_criterion);
      ...
      if (bucket_idx_right >= 0 && node_idx_right >= 0) {
        ...
        term *= node_fluid_vol_x[bucket_idx_right][node_idx_right] *
                    (1.0 - node_psi_x[bucket_idx_right][node_idx_right]) *
                    node_inv_mdv_x[bucket_idx_right][node_idx_right] +
                node_elasto_vol_x[bucket_idx_right][node_idx_right] *
                    node_psi_x[bucket_idx_right][node_idx_right] *
                    node_inv_mdvs_x[bucket_idx_right][node_idx_right];
        ...
        if (bucket_pressure_right >= 0 && node_pressure_right >= 0) {
          ...
          if (right_phi < 0.0) {
            bucket_out(i) +=
                term * (node_vec[bucket_idx][i] -
                        node_vec[bucket_pressure_right][node_pressure_right]);
          } else {
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, right_phi),
                         theta_criterion);
      ...
      if (bucket_idx_bottom >= 0 && node_idx_bottom >= 0) {
        ...
        term *= node_fluid_vol_y[bucket_idx_bottom][node_idx_bottom] *
                    (1.0 - node_psi_y[bucket_idx_bottom][node_idx_bottom]) *
                    node_inv_mdv_y[bucket_idx_bottom][node_idx_bottom] +
                node_elasto_vol_y[bucket_idx_bottom][node_idx_bottom] *
                    node_psi_y[bucket_idx_bottom][node_idx_bottom] *
                    node_inv_mdvs_y[bucket_idx_bottom][node_idx_bottom];
        ...
        if (bucket_pressure_bottom >= 0 && node_pressure_bottom >= 0) {
          ...
          if (bottom_phi < 0.0) {
            bucket_out(i) +=
                term * (node_vec[bucket_idx][i] -
                        node_vec[bucket_pressure_bottom][node_pressure_bottom]);
          } else {
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, bottom_phi),
                         theta_criterion);
      ...
      if (bucket_idx_top >= 0 && node_idx_top >= 0) {
        ...
        term *= node_fluid_vol_y[bucket_idx_top][node_idx_top] *
                    (1.0 - node_psi_y[bucket_idx_top][node_idx_top]) *
                    node_inv_mdv_y[bucket_idx_top][node_idx_top] +
                node_elasto_vol_y[bucket_idx_top][node_idx_top] *
                    node_psi_y[bucket_idx_top][node_idx_top] *
                    node_inv_mdvs_y[bucket_idx_top][node_idx_top];
        ...
        if (bucket_pressure_top >= 0 && node_pressure_top >= 0) {
          ...
          if (top_phi < 0.0) {
            bucket_out(i) +=
                term * (node_vec[bucket_idx][i] -
                        node_vec[bucket_pressure_top][node_pressure_top]);
          } else {
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, top_phi),
                         theta_criterion);
      ...
      if (bucket_idx_near >= 0 && node_idx_near >= 0) {
        ...
        term *= node_fluid_vol_z[bucket_idx_near][node_idx_near] *
                    (1.0 - node_psi_z[bucket_idx_near][node_idx_near]) *
                    node_inv_mdv_z[bucket_idx_near][node_idx_near] +
                node_elasto_vol_z[bucket_idx_near][node_idx_near] *
                    node_psi_z[bucket_idx_near][node_idx_near] *
                    node_inv_mdvs_z[bucket_idx_near][node_idx_near];
        ...
        if (bucket_pressure_near >= 0 && node_pressure_near >= 0) {
          ...
          if (near_phi < 0.0) {
            bucket_out(i) +=
                term * (node_vec[bucket_idx][i] -
                        node_vec[bucket_pressure_near][node_pressure_near]);
          } else {
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, near_phi),
                         theta_criterion);
      ...
      if (bucket_idx_far >= 0 && node_idx_far >= 0) {
        ...
        term *= node_fluid_vol_z[bucket_idx_far][node_idx_far] *
                    (1.0 - node_psi_z[bucket_idx_far][node_idx_far]) *
                    node_inv_mdv_z[bucket_idx_far][node_idx_far] +
                node_elasto_vol_z[bucket_idx_far][node_idx_far] *
                    node_psi_z[bucket_idx_far][node_idx_far] *
                    node_inv_mdvs_z[bucket_idx_far][node_idx_far];
        ...
        if (bucket_pressure_far >= 0 && node_pressure_far >= 0) {
          ...
          if (far_phi < 0.0) {
            bucket_out(i) +=
                term * (node_vec[bucket_idx][i] -
                        node_vec[bucket_pressure_far][node_pressure_far]);
          } else {
            const scalar theta =
                std::max(mathutils::fraction_inside(center_phi, far_phi),
                         theta_criterion);
...
void allocateNodes(const TwoDScene& scene,
                   std::vector<VectorXs>& out_node_vec) {
  const Sorter& buckets = scene.getParticleBuckets();
  const std::vector<VectorXi>& pressure_neighbors =
      scene.getPressureNeighbors();
...
void localSolveJacobi(const std::vector<VectorXs>& node_vec,
  ...
  threadutils::for_each(0, num_buckets, [&](int bucket_idx) {
    out_node_vec[bucket_idx] =
        node_vec[bucket_idx].array() * jacobi_precond[bucket_idx].array();
  });
}
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_rhs_x; ++i) {
      ...
      if (bucket_weight_x(i) > 0.) {
        ...
        if (indices[0] >= 0 && indices[1] >= 0)
          left_phi = node_liquid_phi[indices[0]][indices[1]];

        if (indices[2] >= 0 && indices[3] >= 0)
          right_phi = node_liquid_phi[indices[2]][indices[3]];

        if (left_phi < 0.0 || right_phi < 0.0) {
          scalar theta = 1.;
          if (right_phi >= 0.0 || left_phi >= 0.0)
            theta = std::max(mathutils::fraction_inside(left_phi, right_phi),
                             theta_criterion);

          scalar pressure_grad = 0;
          if (indices[0] >= 0 && indices[1] >= 0)
            pressure_grad -= pressure_vec[indices[0]][indices[1]];

          if (indices[2] >= 0 && indices[3] >= 0)
            pressure_grad += pressure_vec[indices[2]][indices[3]];

          pressure_grad /= (dx * theta);

          scalar term =
              pressure_grad * dt *
              (node_vol_fluid_x[bucket_idx][i] +
               node_vol_x[bucket_idx][i] * node_mshdvm_hdvm_x[bucket_idx][i]);
    ...
    for (int i = 0; i < num_rhs_y; ++i) {
      ...
      if (bucket_weight_y(i) > 0.) {
        ...
        if (indices[0] >= 0 && indices[1] >= 0)
          bottom_phi = node_liquid_phi[indices[0]][indices[1]];

        if (indices[2] >= 0 && indices[3] >= 0)
          top_phi = node_liquid_phi[indices[2]][indices[3]];

        if (bottom_phi < 0.0 || top_phi < 0.0) {
          scalar theta = 1.;
          if (top_phi >= 0.0 || bottom_phi >= 0.0)
            theta = std::max(mathutils::fraction_inside(bottom_phi, top_phi),
                             theta_criterion);
          ...
          scalar term =
              pressure_grad * dt *
              (node_vol_fluid_y[bucket_idx][i] +
               node_vol_y[bucket_idx][i] * node_mshdvm_hdvm_y[bucket_idx][i]);
    ...
    for (int i = 0; i < num_rhs_z; ++i) {
      ...
      if (bucket_weight_z(i) > 0.) {
        ...
        if (indices[0] >= 0 && indices[1] >= 0)
          near_phi = node_liquid_phi[indices[0]][indices[1]];

        if (indices[2] >= 0 && indices[3] >= 0)
          far_phi = node_liquid_phi[indices[2]][indices[3]];

        if (near_phi < 0.0 || far_phi < 0.0) {
          scalar theta = 1.;
          if (far_phi >= 0.0 || near_phi >= 0.0)
            theta = std::max(mathutils::fraction_inside(near_phi, far_phi),
                             theta_criterion);
          ...
          scalar term =
              pressure_grad * dt *
              (node_vol_fluid_z[bucket_idx][i] +
               node_vol_z[bucket_idx][i] * node_mshdvm_hdvm_z[bucket_idx][i]);
      ...
      if (bucket_weight_x(i) > 0.) {
        ...
        if (left_phi < 0.0 || right_phi < 0.0) {
          ...
          scalar term = pressure_grad * dt *
                        (node_vol_x[bucket_idx][i] +
                         node_vol_fluid_x[bucket_idx][i] *
                             node_mfhdvm_hdvm_x[bucket_idx][i]);
    ...
    for (int i = 0; i < num_rhs_y; ++i) {
      if (bucket_weight_y(i) > 0.) {
        ...
        if (bottom_phi < 0.0 || top_phi < 0.0) {
          ...
          scalar term = pressure_grad * dt *
                        (node_vol_y[bucket_idx][i] +
                         node_vol_fluid_y[bucket_idx][i] *
                             node_mfhdvm_hdvm_y[bucket_idx][i]);
    ...
    for (int i = 0; i < num_rhs_z; ++i) {
      if (bucket_weight_z(i) > 0.) {
        ...
        if (near_phi < 0.0 || far_phi < 0.0) {
          ...
          scalar term = pressure_grad * dt *
                        (node_vol_z[bucket_idx][i] +
                         node_vol_fluid_z[bucket_idx][i] *
                             node_mfhdvm_hdvm_z[bucket_idx][i]);
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_rhs_x; ++i) {
      if (bucket_weight_x(i) > 0.) {
        ...
        const scalar left_phi = (indices[0] < 0 || indices[1] < 0)
                                    ? node_liquid_phi[indices[2]][indices[3]]
                                    : node_liquid_phi[indices[0]][indices[1]];
        const scalar right_phi = (indices[2] < 0 || indices[3] < 0)
                                     ? node_liquid_phi[indices[0]][indices[1]]
                                     : node_liquid_phi[indices[2]][indices[3]];

        if (left_phi * right_phi <= 0.0) {
          scalar theta = std::max(
              mathutils::fraction_inside(left_phi, right_phi), theta_criterion);
    ...
    for (int i = 0; i < num_rhs_y; ++i) {
      if (bucket_weight_y(i) > 0.) {
        ...
        const scalar left_phi =
            (indices[1] >= node_liquid_phi[indices[0]].size())
                ? 0.5 * dx
                : node_liquid_phi[indices[0]][indices[1]];
        const scalar right_phi =
            (indices[3] >= node_liquid_phi[indices[2]].size())
                ? 0.5 * dx
                : node_liquid_phi[indices[2]][indices[3]];
        ...
        for (int j = 0; j < num_nodes; ++j)
          for (int i = 0; i < num_nodes; ++i) {
            const int center_cell_idx =
                k * num_nodes * num_nodes + j * num_nodes + i;
            ...
            for (int r = 0; r < 6; ++r) {
              ...
              if (node_bucket_handle(0) < 0 ||
                  node_bucket_handle(0) >= buckets.dim_size(0) ||
                  node_bucket_handle(1) < 0 ||
                  node_bucket_handle(1) >= buckets.dim_size(1) ||
                  node_bucket_handle(2) < 0 ||
                  node_bucket_handle(2) >= buckets.dim_size(2))
                continue;

              const int node_bucket_idx =
                  buckets.bucket_index(node_bucket_handle);
              if (!scene.isBucketActivated(node_bucket_idx)) continue;

              const int neigh_cell_idx = neigh_idx(2) * num_nodes * num_nodes +
                                         neigh_idx(1) * num_nodes +
                                         neigh_idx(0);
