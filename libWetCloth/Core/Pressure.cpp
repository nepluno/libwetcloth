	...
	buckets.for_each_bucket([&] (int bucket_idx) {
		...
		for (int i = 0; i < num_rhs_x; ++i) {
			...
			if (bucket_weight_x(i) > 0. ) {
				...
				if (indices[0] >= 0 && indices[1] >= 0) left_phi = node_liquid_phi[indices[0]][indices[1]];

				if (indices[2] >= 0 && indices[3] >= 0) right_phi = node_liquid_phi[indices[2]][indices[3]];
		...
		for (int i = 0; i < num_rhs_y; ++i) {
			...
			if (bucket_weight_y(i) > 0. ) {
				...
				if (indices[0] >= 0 && indices[1] >= 0) bottom_phi = node_liquid_phi[indices[0]][indices[1]];

				if (indices[2] >= 0 && indices[3] >= 0) top_phi = node_liquid_phi[indices[2]][indices[3]];
		...
		for (int i = 0; i < num_rhs_z; ++i) {
			...
			if (bucket_weight_z(i) > 0. ) {
				...
				if (indices[0] >= 0 && indices[1] >= 0) near_phi = node_liquid_phi[indices[0]][indices[1]];

				if (indices[2] >= 0 && indices[3] >= 0) far_phi = node_liquid_phi[indices[2]][indices[3]];
