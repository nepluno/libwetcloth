...
template <typename T>
inline void check_isnan(const char* name,
...
inline scalar grad_N_kernel_x(const Vector3s& x, const scalar& h,
                              const int order) {
...
inline scalar grad_N_kernel_y(const Vector3s& x, const scalar& h,
                              const int order) {
  std::function<scalar(const scalar&)> val =
      (order == 1) ? linear_kernel
                   : ((order == 3) ? cubic_kernel : quad_kernel);
...
inline scalar grad_N_kernel_z(const Vector3s& x, const scalar& h,
                              const int order) {
  std::function<scalar(const scalar&)> val =
      (order == 1) ? linear_kernel
                   : ((order == 3) ? cubic_kernel : quad_kernel);
...
inline scalar defaultRadiusMultiplier() { return 1.0 / sqrt(3.0); }
...
inline void grad_triangle(const Vector3s& x0, const Vector3s& x1,
                          const Vector3s& x2, Matrix3s& grad_coeff) {
...
inline void get_div_triangle(const scalar& va0, const scalar& va1,
                             ...
                             const Vector3s& x2, Vector3s& div0, Vector3s& div1,
                             Vector3s& div2) {
  Vector3s n = (x2 - x0).cross(x1 - x0);
  const scalar len = n.norm();
  if (len < 1e-63) {
...
inline scalar interpolate_2nd_order(const Vector3s& p, const Array3T& arr,
                                    Callable func) {
  Vector3s frac_p =
      Vector3s(p(0) - floor(p(0)), p(1) - floor(p(1)), p(2) - floor(p(2)));
  Vector3i base_p = Vector3i((int)floor(p(0)) + (frac_p(0) < 0.5 ? -1 : 0),
                             (int)floor(p(1)) + (frac_p(1) < 0.5 ? -1 : 0),
                             (int)floor(p(2)) + (frac_p(2) < 0.5 ? -1 : 0));

  Vector3s local_p =
      frac_p + Vector3s(frac_p(0) < 0.5 ? 1 : 0, frac_p(1) < 0.5 ? 1 : 0,
                        frac_p(2) < 0.5 ? 1 : 0);
...
template <typename Callable, typename Array3T>
inline scalar interpolate_2nd_order_with_grad(const Vector3s& p, Vector3sT& B,
                                              const Array3T& arr,
                                              const scalar& h, Callable func) {
  Vector3s frac_p =
      Vector3s(p(0) - floor(p(0)), p(1) - floor(p(1)), p(2) - floor(p(2)));
  Vector3i base_p = Vector3i((int)floor(p(0)) + (frac_p(0) < 0.5 ? -1 : 0),
                             (int)floor(p(1)) + (frac_p(1) < 0.5 ? -1 : 0),
...
inline void dhdr_yarn(double mu, double la, double r22, double r23, double r33,
                      double* dhdr22, double* dhdr23, double* dhdr33) {
...
inline void dgdr_cloth(double mu, double r13, double r23, double* dhdr13,
                       double* dhdr23) {
  *dhdr13 = mu * r13;
  *dhdr23 = mu * r23;
}
...
void print_histogram_analysis(const std::vector<VectorXs>&, int,
